...
![Executive P&L Overview](screenshots/Executive%20P%26L%20Overview.png)
...
![Budget vs. Actual](screenshots/Budget%20vs.%20Actual.png)
...
![Cash Flow & Runway](screenshots/Cash%20Flow%20%26%20Runway.png)
...
![AR Aging & Collections](screenshots/AR%20Aging%20%26%20Collections.png)
...
**Virtual relationship bridge** (forces a physical-relationship-free link between accounts and GL transactions):
```dax
Total OpEx =
CALCULATE (
    ABS ( SUM ( Fact_GL_Actual[Amount] ) ),
    Dim_Account[AccountCategory] IN { "Finance", "General & Administrative", "Personnel",
                                        "Research & Development", "Sales & Marketing" },
    TREATAS ( VALUES ( Dim_Account[AccountID] ), Fact_GL_Actual[AccountID] )
)
```

**Dynamic AR aging with a cross-filtered TREATAS bridge + adaptive DATEDIFF baseline:**
```dax
DSO (Days) =
...
**Cash runway, driven off actual monthly burn rate:**
```dax
Runway (Months) =
